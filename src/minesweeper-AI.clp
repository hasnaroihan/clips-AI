...
(defrule update-value1
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x+1 ?y+1 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x+1 ?y+1 (+ 1 ?val))))
(defrule update-value2
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x+1 ?y+0 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x+1 ?y+0 (+ 1 ?val))))
(defrule update-value3
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x+1 ?y-1 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x+1 ?y-1 (+ 1 ?val))))
(defrule update-value4
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x+0 ?y+1 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x+0 ?y+1 (+ 1 ?val))))
(defrule update-value5
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x+0 ?y-1 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x+0 ?y-1 (+ 1 ?val))))
(defrule update-value6
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x-1 ?y+1 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x-1 ?y+1 (+ 1 ?val))))
(defrule update-value7
    (bomb ?x ?y)
    ?f <- (jumlah-bom-xy ?x-1 ?y+0 ?val)
    =>
    (modify ?f (jumlah-bom-xy ?x-1 ?y+0 (+ 1 ?val))))
